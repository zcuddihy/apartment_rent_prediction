...
############# Not used currently #############
def unit_exists(conn, property_name: str, unit_id: str, date_scraped: str):
    """Checks if the unit for the scraped date already exists in the database"""
    c = conn.cursor()
    query = f"""SELECT EXISTS(SELECT 1 
                FROM properties 
                WHERE property_name="{property_name}"
                AND unit_id="{unit_id}"
                AND date_scraped="{date_scraped}"
                LIMIT 1);"""
